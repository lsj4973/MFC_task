...
BEGIN_MESSAGE_MAP(CCodingTestDlg, CDialogEx)
	...
	ON_WM_QUERYDRAGICON()
	ON_WM_DESTROY()
	ON_BN_CLICKED(IDC_BTN_GET_DATA, &CCodingTestDlg::OnBnClickedBtnGetData)
	ON_BN_CLICKED(IDC_BTN_DRAW_FIGURE, &CCodingTestDlg::OnBnClickedBtnDrawFigure)
END_MESSAGE_MAP()
...
void CCodingTestDlg::OnBnClickedBtnDrawFigure()
{
	CString str;
	int nRadius;
...
void CCodingTestDlg::DrawFigure(int nRadius)
{
	...
	int nSelFigure = rand() % 3;
	m_pDlgImage->m_nSelColor = nSelFigure;

	// 원 테두리 좌표 배열에 저장
	SetBorderPoint(nCenterX, nCenterY, nRadius, nSelFigure);
...
void CCodingTestDlg::SetBorderPoint(int nCenterX, int nCenterY, int nRadius, int nSelFigure)
{
	for (int j = nCenterY - nRadius; j <= nCenterY + nRadius; j++) {
		for (int i = nCenterX - nRadius; i <= nCenterX + nRadius; i++) {
...
bool CCodingTestDlg::IsBorderPoint(int x, int y, int centerX, int centerY, int radius, int selFigure)
{
	switch (selFigure) {
	case 0:
...
// 무게 중심 구하기
void CCodingTestDlg::OnBnClickedBtnGetData()
{
	unsigned char* fm = (unsigned char*)m_pDlgImage->m_image.GetBits();
	int nWidth = m_pDlgImage->m_image.GetWidth();
	int nHeight = m_pDlgImage->m_image.GetHeight();
